...
def enterSite():
	...
	if(request.method=='POST'):
		if(request.form['btn']=='login'):
			...
			r = requests.post(api_url+'api/v1/users/authenticate',json={"username":request.form['username'],"password":pwd_sha1.hexdigest()})
			...
			pwd_sha1 = sha1(request.form['password'].encode())
			r = requests.post(api_url+'api/v1/users',json={"username":request.form['username'],"password":pwd_sha1.hexdigest()})
...
def dbUpdate():
	...
	r= requests.get(api_url+'api/v1/categories/feed/acts')
	...
	if(request.method=='POST'):
		try:
			if(request.form['btn']=='add_cat'):
				r = requests.post(api_url+'api/v1/categories',json=[request.form['category']])
			...
			if(request.form['btn']=='rem_cat'):
				...
				r = requests.delete(api_url+url_format)
			...
			if(request.form['btn'] == 'upload'):
				...
				r = requests.post(api_url+'api/v1/acts',json=request_body)
				return(redirect(url_for('dbUpdate')))
			...
			if(request.form['btn_del']):
				...
				r = requests.post(api_url+url_format,json={"actId":int(request.form['btn_del']),"username":session['username']})
				if(r.status_code==200):
					url_format='api/v1/acts/{}'.format(request.form['btn_del'])
					del_act = requests.delete(api_url+url_format)
					return(redirect(url_for('dbUpdate')))
				else:
...
def updateFeed(username):
	...
	r= requests.post(api_url+api_format)
	if(r.status_code==200):
		posts = json.loads(r.text)
	else:
		posts=[]

	r = requests.get(api_url+api_format)
	categories = json.loads(r.text)
